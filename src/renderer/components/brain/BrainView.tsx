  ...
  return (
    <div style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, display: 'flex', flexDirection: 'column', background: 'var(--bg-primary)' }}>
      ...
      <div className="flex items-center gap-0 px-4 border-b" style={{ borderColor: 'var(--border-color)' }}>
        ...
        {activeTab === 'graph' && (
          <>
            <button onClick={() => (window as any).kyberbot?.brain?.popout()} className="px-2 py-1 text-[11px] tracking-[1px] uppercase" style={{ fontFamily: 'var(--font-mono)', color: 'var(--accent-violet)', background: 'transparent', border: 'none', cursor: 'pointer' }}>Pop Out</button>
            <button onClick={loadGraph} className="px-2 py-1 text-[11px] tracking-[1px] uppercase" style={{ fontFamily: 'var(--font-mono)', color: 'var(--fg-muted)', background: 'transparent', border: 'none', cursor: 'pointer' }}>Refresh</button>
          </>
        )}
        <span className="text-[11px] ml-2" style={{ color: 'var(--fg-muted)', fontFamily: 'var(--font-mono)' }}>
        ...
        {activeTab === 'graph' && (
          loading ? (
            <div style={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <span style={{ fontSize: '12px', color: 'var(--fg-muted)', fontFamily: 'var(--font-mono)' }}>Loading graph...</span>
            </div>
          ) : (
...
function BrainNotesView({ serverUrl, apiToken, serverReady }: { serverUrl: string; apiToken: string | null; serverReady: boolean }) {
    ...
    return (
      <div style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, display: 'flex', flexDirection: 'column' }}>
        <div style={{ padding: '8px 16px', borderBottom: '1px solid var(--border-color)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <span style={{ fontSize: '12px', fontFamily: 'var(--font-mono)', color: 'var(--accent-emerald)' }}>{selectedNote.name}</span>
          <button onClick={() => setSelectedNote(null)} style={{ fontSize: '11px', letterSpacing: '1px', textTransform: 'uppercase', fontFamily: 'var(--font-mono)', color: 'var(--fg-muted)', background: 'transparent', border: 'none', cursor: 'pointer' }}>Back</button>
        </div>
  ...
  return (
    <div style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, overflowY: 'auto', padding: '16px' }}>
      ...
      <p style={{ fontSize: '12px', color: 'var(--fg-muted)', fontFamily: 'var(--font-mono)', marginTop: '4px', marginBottom: '12px' }}>
        {notes.length} files across brain notes, Claude Code memory, identity files, and memory sync
      </p>

      {/* Source filter buttons */}
      <div style={{ display: 'flex', gap: '4px', marginBottom: '12px', flexWrap: 'wrap' }}>
        <button onClick={() => setFilter(null)} style={{ padding: '2px 8px', fontSize: '10px', letterSpacing: '0.5px', textTransform: 'uppercase', fontFamily: 'var(--font-mono)', border: `1px solid ${!filter ? 'var(--accent-emerald)' : 'var(--border-color)'}`, color: !filter ? 'var(--accent-emerald)' : 'var(--fg-muted)', background: !filter ? 'rgba(16,185,129,0.1)' : 'transparent', cursor: 'pointer' }}>All ({notes.length})</button>
        {sources.map(src => {
          const meta = SOURCE_LABELS[src] || { label: src, color: 'var(--fg-muted)' };
          const count = notes.filter(n => n.source === src).length;
          return (
            <button key={src} onClick={() => setFilter(filter === src ? null : src)} style={{ padding: '2px 8px', fontSize: '10px', letterSpacing: '0.5px', textTransform: 'uppercase', fontFamily: 'var(--font-mono)', border: `1px solid ${filter === src ? meta.color : 'var(--border-color)'}`, color: filter === src ? meta.color : 'var(--fg-muted)', background: filter === src ? `${meta.color}15` : 'transparent', cursor: 'pointer' }}>{meta.label} ({count})</button>
          );
        })}
      </div>

      {loading && <span style={{ fontSize: '12px', color: 'var(--fg-muted)', fontFamily: 'var(--font-mono)' }}>Loading...</span>}
      {filtered.length === 0 && !loading && <span style={{ fontSize: '12px', color: 'var(--fg-muted)', fontFamily: 'var(--font-mono)' }}>No files in this category</span>}
          ...
          return (
            <div key={note.path} onClick={() => openNote(note.path)} style={{ padding: '10px', border: '1px solid var(--border-color)', background: 'var(--bg-secondary)', cursor: 'pointer' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px' }}>
                <span style={{ fontSize: '10px', padding: '1px 4px', textTransform: 'uppercase', fontFamily: 'var(--font-mono)', color: meta.color, background: `${meta.color}15`, border: `1px solid ${meta.color}30` }}>{meta.label}</span>
                <span style={{ fontSize: '12px', fontFamily: 'var(--font-mono)', color: 'var(--fg-primary)', flex: 1 }}>{note.name}</span>
                <span style={{ fontSize: '11px', color: 'var(--fg-muted)', fontFamily: 'var(--font-mono)' }}>{(note.size / 1024).toFixed(1)}KB</span>
              </div>
...
function TimelineView({ serverUrl, apiToken, serverReady }: { serverUrl: string; apiToken: string | null; serverReady: boolean }) {
  ...
  return (
    <div style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, overflowY: 'auto', padding: '16px' }}>
      ...
      <p style={{ fontSize: '12px', color: 'var(--fg-muted)', fontFamily: 'var(--font-mono)', marginTop: '4px', marginBottom: '16px' }}>
  ...
  return (
    <div style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, display: 'flex', flexDirection: 'column' }}>
      <div style={{ padding: '12px 16px', borderBottom: '1px solid var(--border-color)', display: 'flex', gap: '8px' }}>
        ...
        />
        <button onClick={search} style={{ padding: '4px 12px', fontSize: '11px', letterSpacing: '1px', textTransform: 'uppercase', fontFamily: 'var(--font-mono)', background: 'var(--accent-emerald)', color: '#ffffff', border: '1px solid var(--accent-emerald)', cursor: 'pointer' }}>Search</button>
      </div>
      <div style={{ flex: 1, overflowY: 'auto', padding: '16px' }}>
        {loading && <span style={{ fontSize: '12px', color: 'var(--fg-muted)', fontFamily: 'var(--font-mono)' }}>Searching...</span>}
        {results.map((r, i) => {
          const isExpanded = expanded.has(i);
          ...
          return (
            <div key={i} onClick={() => toggle(i)} style={{ padding: '10px', borderBottom: '1px solid var(--border-color)', cursor: 'pointer' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px' }}>
                {r.type && <span style={{ fontSize: '10px', padding: '1px 4px', textTransform: 'uppercase', fontFamily: 'var(--font-mono)', color: r.type === 'conversation' ? 'var(--accent-cyan)' : r.type === 'note' ? 'var(--accent-emerald)' : 'var(--accent-violet)', background: r.type === 'conversation' ? 'rgba(34,211,238,0.1)' : r.type === 'note' ? 'rgba(16,185,129,0.1)' : 'rgba(139,92,246,0.1)' }}>{r.type}</span>}
                <span style={{ fontSize: '12px', fontFamily: 'var(--font-mono)', color: 'var(--fg-primary)', flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{title}</span>
                {r.score != null && <span style={{ fontSize: '10px', color: 'var(--fg-muted)', fontFamily: 'var(--font-mono)' }}>{(r.score * 100).toFixed(0)}%</span>}
                <span style={{ fontSize: '10px', color: 'var(--fg-muted)' }}>{isExpanded ? '\u25BE' : '\u25B8'}</span>
              </div>
              {r.entities && r.entities.length > 0 && (
                <div style={{ display: 'flex', gap: '4px', marginBottom: '4px', flexWrap: 'wrap' }}>
                  {r.entities.slice(0, 5).map((e: string, j: number) => (
                    <span key={j} style={{ fontSize: '10px', padding: '1px 4px', color: 'var(--accent-amber)', background: 'rgba(245,158,11,0.1)', fontFamily: 'var(--font-mono)' }}>{e}</span>
                  ))}
                </div>
              )}
              <div style={{ fontSize: '12px', color: 'var(--fg-secondary)', fontFamily: 'var(--font-mono)', whiteSpace: isExpanded ? 'pre-wrap' : 'nowrap', overflow: isExpanded ? 'visible' : 'hidden', textOverflow: isExpanded ? 'unset' : 'ellipsis', wordBreak: isExpanded ? 'break-word' : 'normal' }}>
                {isExpanded ? content : content.slice(0, 200)}
              </div>
              {isExpanded && r.created_at && (
                <div style={{ fontSize: '10px', color: 'var(--fg-muted)', fontFamily: 'var(--font-mono)', marginTop: '8px' }}>
        ...
        {!loading && results.length === 0 && query && <span style={{ fontSize: '12px', color: 'var(--fg-muted)', fontFamily: 'var(--font-mono)' }}>No results found</span>}
        {!query && <span style={{ fontSize: '12px', color: 'var(--fg-muted)', fontFamily: 'var(--font-mono)' }}>Enter a query to search across all memories — conversations, notes, entity facts, and brain documents</span>}
      </div>
    </div>
  );
}
    ...
    <div style={{ position: 'absolute', top: 0, right: 0, bottom: 0, width: '340px', borderLeft: '1px solid var(--border-color)', overflowY: 'auto', background: 'var(--bg-primary)', zIndex: 10 }}>
      <div style={{ padding: '16px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '12px' }}>
          ...
          <button onClick={onClose} style={{ fontSize: '11px', letterSpacing: '1px', textTransform: 'uppercase', fontFamily: 'var(--font-mono)', color: 'var(--fg-muted)', background: 'transparent', border: 'none', cursor: 'pointer' }}>Close</button>
        </div>

        {/* Name + Type */}
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px' }}>
          <div style={{ fontSize: '16px', fontWeight: 'bold', fontFamily: 'var(--font-mono)', color: 'var(--fg-primary)' }}>{entity.name}</div>
          <span style={{ fontSize: '10px', padding: '1px 6px', textTransform: 'uppercase', fontFamily: 'var(--font-mono)', color: typeColor, background: `${typeColor}15`, border: `1px solid ${typeColor}40` }}>{entity.type}</span>
        </div>
        {entity.tier && <span style={{ fontSize: '10px', padding: '1px 6px', textTransform: 'uppercase', fontFamily: 'var(--font-mono)', color: 'var(--status-warning)', background: 'rgba(245,158,11,0.1)' }}>{entity.tier}</span>}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginTop: '12px', marginBottom: '16px' }}>
          {[['MENTIONS', entity.mention_count], ['PRIORITY', `${(entity.priority * 100).toFixed(0)}%`], ['FIRST SEEN', context?.entity?.first_seen ? new Date(context.entity.first_seen).toLocaleDateString() : '—'], ['LAST SEEN', new Date(entity.last_seen).toLocaleDateString()]].map(([label, value]) => (
            <div key={label as string}>
              <div style={{ fontSize: '10px', letterSpacing: '1px', textTransform: 'uppercase', color: 'var(--fg-muted)', fontFamily: 'var(--font-mono)', marginBottom: '2px' }}>{label}</div>
        ...
        <div style={{ marginBottom: '16px' }}>
          <div style={{ fontSize: '10px', letterSpacing: '1px', textTransform: 'uppercase', color: 'var(--fg-muted)', fontFamily: 'var(--font-mono)', marginBottom: '4px' }}>CONFIDENCE</div>
        ...
        {loading && <div style={{ fontSize: '12px', color: 'var(--fg-muted)', fontFamily: 'var(--font-mono)' }}>Loading details...</div>}
          ...
          <>
            ...
            {context.contradictions && context.contradictions.length > 0 && (
              <div style={{ marginBottom: '16px', padding: '8px', border: '1px solid rgba(245,158,11,0.3)', background: 'rgba(245,158,11,0.05)' }}>
                <div style={{ fontSize: '11px', letterSpacing: '1.5px', textTransform: 'uppercase', color: '#f59e0b', fontFamily: 'var(--font-mono)', marginBottom: '8px' }}>{`// CONTRADICTIONS (${context.contradictions.length})`}</div>
                {context.contradictions.map((c: any, i: number) => (
                  <div key={i} style={{ fontSize: '12px', color: 'var(--fg-secondary)', marginBottom: '4px' }}>{c.description || c.fact_a || JSON.stringify(c).slice(0, 150)}</div>
                ))}
              </div>
            )}
            ...
            {context.facts && context.facts.length > 0 && (
              <div style={{ marginBottom: '16px' }}>
                <div style={{ fontSize: '11px', letterSpacing: '1.5px', textTransform: 'uppercase', color: 'var(--accent-emerald)', fontFamily: 'var(--font-mono)', marginBottom: '8px' }}>{`// FACTS (${context.facts.length})`}</div>
                {context.facts.map((f: any, i: number) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: '6px', paddingBottom: '6px', marginBottom: '6px', borderBottom: '1px solid var(--border-color)' }}>
                    <span style={{ fontSize: '10px', padding: '1px 4px', textTransform: 'uppercase', fontFamily: 'var(--font-mono)', color: f.source_type === 'corrected' ? '#10b981' : f.source_type === 'confirmed' ? '#22d3ee' : 'var(--fg-muted)', background: f.source_type === 'corrected' ? 'rgba(16,185,129,0.1)' : f.source_type === 'confirmed' ? 'rgba(34,211,238,0.1)' : 'var(--bg-tertiary)', flexShrink: 0, marginTop: '2px' }}>{(f.source_type || 'fact').slice(0, 5)}</span>
                    <span style={{ fontSize: '12px', color: 'var(--fg-secondary)' }}>{f.content || f.fact || JSON.stringify(f).slice(0, 200)}</span>
                  </div>
                ))}
              </div>
            )}
            ...
            {context.related_entities && context.related_entities.length > 0 && (
              <div style={{ marginBottom: '16px' }}>
                <div style={{ fontSize: '11px', letterSpacing: '1.5px', textTransform: 'uppercase', color: 'var(--accent-cyan)', fontFamily: 'var(--font-mono)', marginBottom: '8px' }}>{`// RELATIONSHIPS (${context.related_entities.length})`}</div>
                {context.related_entities.map((rel: any, i: number) => {
                  ...
                  return (
                    <div key={i} onClick={() => relEntity.id && onNavigate(relEntity.id)} style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '4px 0', cursor: 'pointer' }}>
                      <span style={{ fontSize: '10px', padding: '1px 4px', fontFamily: 'var(--font-mono)', color: 'var(--accent-violet)', background: 'rgba(139,92,246,0.1)', border: '1px solid rgba(139,92,246,0.3)' }}>{rel.relationship || 'related'}</span>
                      <span style={{ fontSize: '12px', fontFamily: 'var(--font-mono)', color: relColor }}>{relName}</span>
                      <span style={{ fontSize: '10px', color: 'var(--fg-muted)', fontFamily: 'var(--font-mono)' }}>({relType})</span>
                      {rel.strength && <span style={{ fontSize: '10px', color: 'var(--fg-muted)', fontFamily: 'var(--font-mono)', marginLeft: 'auto' }}>×{rel.strength}</span>}
                    </div>
                  );
                })}
              </div>
            ...
            {context.mentions && context.mentions.length > 0 && (
              <div>
                <div style={{ fontSize: '11px', letterSpacing: '1.5px', textTransform: 'uppercase', color: '#14b8a6', fontFamily: 'var(--font-mono)', marginBottom: '8px' }}>{`// MENTIONS (${context.mentions.length})`}</div>
                {context.mentions.slice(0, 15).map((m: any, i: number) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: '6px', paddingBottom: '6px' }}>
                    ...
                    <div>
                      <div style={{ fontSize: '10px', color: 'var(--fg-muted)', fontFamily: 'var(--font-mono)' }}>{new Date(m.timestamp || m.created_at).toLocaleString()}</div>
                      <div style={{ fontSize: '11px', color: 'var(--fg-secondary)' }}>{(m.context || m.content || '').slice(0, 200)}</div>
                    </div>
                  </div>
                ))}
              </div>
