      ...
      <div className="h-full flex flex-col" style={{ background: 'var(--bg-primary)' }}>
        <div className="flex items-center justify-between px-4 py-2 border-b" style={{ borderColor: 'var(--border-color)' }}>
          ...
          <div className="flex gap-2">
            <button onClick={() => setEditing(null)} className="px-3 py-1 text-[11px] tracking-[1px] uppercase border" style={{ fontFamily: 'var(--font-mono)', borderColor: 'var(--fg-muted)', color: 'var(--fg-muted)', background: 'transparent', cursor: 'pointer' }}>Cancel</button>
            <button onClick={saveContent} className="px-3 py-1 text-[11px] tracking-[1px] uppercase border" style={{ fontFamily: 'var(--font-mono)', background: 'var(--accent-emerald)', color: '#ffffff', border: '1px solid var(--accent-emerald)', cursor: 'pointer' }}>Save</button>
          </div>
        </div>
        <textarea
          value={editContent}
  ...
  return (
    <div style={{ position: "absolute", top: 0, left: 0, right: 0, bottom: 0, overflowY: "auto", padding: 16, background: "var(--bg-primary)" }}>
      <div className="flex items-center justify-between mb-4">
        ...
        <button onClick={() => setCreating(!creating)} className="px-3 py-1 text-[11px] tracking-[1px] uppercase border" style={{ fontFamily: 'var(--font-mono)', background: 'var(--accent-emerald)', color: '#ffffff', border: '1px solid var(--accent-emerald)', cursor: 'pointer' }}>
          {creating ? 'Cancel' : '+ New'}
        </button>
      </div>

      {error && <div className="mb-3 p-2 text-[12px] border" style={{ fontFamily: 'var(--font-mono)', borderColor: 'var(--status-error)', color: 'var(--status-error)' }}>{error}</div>}
      ...
      {creating && (
        <div className="mb-4 p-3 border" style={{ background: 'var(--bg-secondary)', borderColor: 'var(--border-color)' }}>
          ...
          <button onClick={createSkill} className="px-3 py-1 text-[11px] tracking-[1px] uppercase border" style={{ fontFamily: 'var(--font-mono)', background: 'var(--accent-emerald)', color: '#ffffff', border: '1px solid var(--accent-emerald)', cursor: 'pointer' }}>Create</button>
        </div>
      )}
          ...
          <div key={skill.name} className="p-3 border flex items-start justify-between" style={{ background: 'var(--bg-secondary)', borderColor: 'var(--border-color)' }}>
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-[11px] px-1" style={{ color: skill.isReady ? 'var(--status-success)' : 'var(--status-warning)', background: skill.isReady ? 'rgba(16,185,129,0.1)' : 'rgba(245,158,11,0.1)' }}>
                  {skill.isReady ? 'READY' : 'NEEDS SETUP'}
                </span>
              </div>
              <span className="text-[13px]" style={{ color: 'var(--fg-secondary)' }}>{skill.description}</span>
            </div>
            <div className="flex gap-1 ml-2">
              <button onClick={() => openEditor(skill.name)} className="px-2 py-1 text-[11px] tracking-[1px] uppercase" style={{ fontFamily: 'var(--font-mono)', color: 'var(--accent-cyan)', background: 'transparent', border: 'none', cursor: 'pointer' }}>Edit</button>
              <button onClick={() => deleteSkill(skill.name)} className="px-2 py-1 text-[11px] tracking-[1px] uppercase" style={{ fontFamily: 'var(--font-mono)', background: 'var(--status-error)', color: '#ffffff', border: 'none', cursor: 'pointer' }}>Del</button>
            </div>
          </div>
        ))}
      </div>
      ...
      {!loading && skills.length === 0 && (
        <div className="text-center py-8">
          <span className="text-[12px]" style={{ color: 'var(--fg-muted)', fontFamily: 'var(--font-mono)' }}>No skills installed</span>
        </div>
      )}
    </div>
  );
