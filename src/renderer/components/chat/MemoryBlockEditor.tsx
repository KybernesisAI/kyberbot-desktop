    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: 'rgba(0,0,0,0.8)', backdropFilter: 'blur(4px)' }}>
      <div className="w-full max-w-2xl border p-8" style={{ background: 'var(--bg-primary)', borderColor: 'var(--border-color)' }}>
        <div className="flex items-center justify-between border-b pb-4 mb-6" style={{ borderColor: 'var(--border-color)' }}>
          <span className="text-[11px] tracking-[1.5px]" style={{ color: 'var(--accent-violet)', fontFamily: 'var(--font-mono)' }}>
        ...
        <div className="flex items-center justify-between mt-4">
          <span className="text-[11px]" style={{ color: 'var(--fg-muted)', fontFamily: 'var(--font-mono)' }}>{editValue.length} CHARS</span>
          <div className="flex gap-3">
            <button onClick={onClose} className="border px-4 py-2 text-[11px] tracking-[1px] uppercase" style={{ fontFamily: 'var(--font-mono)', borderColor: 'var(--border-color)', color: 'var(--fg-secondary)', background: 'transparent', cursor: 'pointer' }}>Cancel</button>
            <button
              onClick={async () => { try { await onSave(editValue); } catch {} }}
              disabled={saving || editValue === content}
              className="border px-4 py-2 text-[11px] tracking-[1px] uppercase"
              style={{ fontFamily: 'var(--font-mono)', background: 'var(--accent-emerald)', color: '#ffffff', border: '1px solid var(--accent-emerald)', cursor: saving || editValue === content ? 'default' : 'pointer', opacity: saving || editValue === content ? 0.3 : 1 }}
            >
              {saving ? 'Saving...' : 'Save'}
            </button>
          </div>
